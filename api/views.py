...
def add_event(request):
    ...
    if result:
        return JsonResponse({'status': 10023,
                             'msg': 'event name is already exists'})
...
def add_guest(request):
    ...
    if not result:
        return JsonResponse({'status': 10023,
                             'msg': 'event status id not available'})
